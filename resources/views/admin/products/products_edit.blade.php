...
@section('content')
    ...
    <div class="page-content">
        ...
        <div class="row">
            <div class="col-xs-12">
                <!-- PAGE CONTENT BEGINS -->
                {!! Form::open(['route' => ['admin_products_edit_post', $product->id], 'class' => 'form-horizontal', 'role' => 'form', 'files' => true]) !!}
                    @if (isset($failed))
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="product_name" id="product_name" placeholder="Nombre" value="{{ $product->name }}" class="col-xs-10 col-sm-5" required/>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <select class="chosen-select form-control" id="product_category_id" name="product_category_id" data-placeholder="Elegir categoria..." required>
                                ...
                                @foreach($product_categories as $product_category)
                                    <option value="{{ $product_category->id }}" {{ $product->product_category->id == $product_category->id ? "selected" : "" }}>{{ $product_category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <select class="chosen-select form-control" id="product_subcategory_id" name="product_subcategory_id" data-placeholder="Elegir subcategoria..." required>
                                ...
                                @foreach($product->product_category->product_subcategories as $product_subcategory)
                                    <option value="{{ $product_subcategory->id }}" {{ !is_null($product->product_subcategory) && $product->product_subcategory->id == $product_subcategory->id ? "selected" : "" }}>{{ $product_subcategory->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <textarea name="product_subtitle" id="product_subtitle" class="autosize-transition form-control" required>{{ $product->subtitle }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <textarea name="product_description" id="product_description" class="autosize-transition form-control" required>{{ $product->description }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <img width="100px" src="{{ Storage::url($product->photo->file_name) }}" /> <br/><br/>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <div class="checkbox">
                                <label>
                                    <input name="product_featured" id="product_featured" type="checkbox" class="ace" {{ $product->featured ? "checked" : "" }} value="1">
                                    <span class="lbl"></span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-9">
                            <select multiple="" name="product_related_courses[]" class="chosen-select form-control" id="form-field-select-4" data-placeholder="Elegir Cursos...">
                                @foreach($courses as $course)
                                    <option value="{{ $course->id }}" {{ !is_null($product->courses->where('id', $course->id)->first()) ? "selected" : "" }}>{{ $course->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group product_related_files" id="form-files">
                        @if($product->files->count() > 0)
                            @foreach($product->files as $index => $relatedFile)
                                <span class="files">
                                    <label class="col-sm-3 control-label no-padding-right"> {{ $index == 0 ? "Archivos Relacionados" : "" }} </label>
                                    <div class="col-sm-9">
                                        <div class="col-sm-3">
                                            <a href="{{ Storage::url($relatedFile->file_name) }}" target="_blank">Descargar {{ $relatedFile->file_name_original }}</a>
                                            <input type="hidden" name="product_related_files_id[]" value="{{ $relatedFile->id }}" />
                                        </div>
                                        @if($index == 0)
