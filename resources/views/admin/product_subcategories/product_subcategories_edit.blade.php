...
@section('content')
    ...
    <div class="page-content">
        ...
        <div class="row">
            <div class="col-xs-12">
                <!-- PAGE CONTENT BEGINS -->
                {!! Form::open(['route' => ['admin_product_subcategories_edit_post', $product_subcategory->id], 'class' => 'form-horizontal', 'role' => 'form']) !!}
                @if (isset($failed))
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="subcategory_name" id="subcategory_name" placeholder="Nombre" class="col-xs-10 col-sm-5" value="{{ $product_subcategory->name }}" required/>
                    </div>
                </div>
                    ...
                    <div class="col-sm-9">
                        <select class="chosen-select form-control" id="subcategory_category_id" name="subcategory_category_id" data-placeholder="Elegir categoria..." required>
                            ...
                            @foreach($product_categories as $product_category)
                                <option value="{{ $product_category->id }}" {{ $product_subcategory->product_category->id == $product_category->id ? "selected" : "" }}>{{ $product_category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
